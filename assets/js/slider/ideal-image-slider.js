...
var IdealImageSlider = (function () {
    ...
    for (var i = 1; i < arguments.length; i++) {
      ...
      for (var key in obj) {
        if (obj.hasOwnProperty(key)) {
          if (_isType("Object", obj[key]) && obj[key] !== null) _deepExtend(out[key], obj[key]);
  ...
  var _touch = {
    ...
    move: function (event) {
      ...
      var isChangeSlide =
        (Number(duration) < 250 && Math.abs(_touch.vars.delta.x) > 20) ||
        Math.abs(_touch.vars.delta.x) > this._attributes.currentSlide.offsetWidth / 2;
